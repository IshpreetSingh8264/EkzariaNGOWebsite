...
const RecipientDetailView = ({ student, onClose }) => {
  ...
  const routerStudent = state?.student;
  
  // Use props student if available (modal mode), otherwise use router student
  const currentStudent = student || routerStudent;
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6">
              <div className="w-full sm:w-auto flex flex-col items-center sm:items-start sm:flex-row gap-2 sm:gap-4">
                ...
                <div className="w-32 h-32 sm:w-36 sm:h-36 md:w-40 md:h-40 rounded-full overflow-hidden border-2 border-gray-200">
                    <img 
                      src={currentStudent.photo.startsWith('data:image') ? currentStudent.photo : `data:image/jpeg;base64,${currentStudent.photo}`}
              ...
              <div className="mt-4 w-full px-0">
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-xs sm:text-sm font-medium text-gray-700">
                    {Math.round(((currentStudent.fundsReceived || 0) / currentStudent.totalAmountNeeded) * 100)}%
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2 sm:h-2.5">
                  <div 
                    ...
                    style={{ 
                      width: `${Math.min(100, ((currentStudent.fundsReceived || 0) / currentStudent.totalAmountNeeded) * 100)}%`,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div>
                ...
                <div className="space-y-3">
                  <p><span className="font-medium">Gender:</span> {currentStudent.gender}</p>
                  <p><span className="font-medium">Email:</span> {currentStudent.email}</p>
                  <p><span className="font-medium">Phone:</span> {currentStudent.phone}</p>
                  <p><span className="font-medium">Address:</span> {currentStudent.location}</p>
                  <p><span className="font-medium">Nationality:</span> {currentStudent.nationality}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <p><span className="font-medium">Education Level:</span> {currentStudent.educationLevel}</p>
                  <p><span className="font-medium">Institution:</span> {currentStudent.institution}</p>
                  <p><span className="font-medium">Field of Study:</span> {currentStudent.fieldOfStudy}</p>
                  <p><span className="font-medium">Achievements:</span> {currentStudent.achievements}</p>
                </div>
              </div>
              ...
              <div className="md:col-span-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p><span className="font-medium">Amount Needed:</span> ${currentStudent.totalAmountNeeded}</p>
                    <p><span className="font-medium">Deadline:</span> {currentStudent.fundingDeadline}</p>
                  </div>
                  <div>
